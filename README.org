...
** WORKING Search for ticket (Estimate ticket screen) [11/12]          :verb:
   :LOGBOOK:
 ...
 - [ ] Async python function with results caching on FastAPI / (memoize | Redis)

** WORKING Voting screen [4/8]
   :LOGBOOK:
 ...
 - [-] Display data on screen
   ...
   - [ ] format images as external URLs via custom fn
 - [X] Add 'vote' layout block
 - [X] Set voted ticket & bg color for button
 - [ ] Send vote data to server
   - [ ] add backend endpoint
     - [ ] store data in dict. [[https://stackoverflow.com/questions/19201290/how-to-save-a-dictionary-to-a-file][use pickle?]]
   - [ ] add fx handler event
   - [ ] send data only if value has changed!
 - [ ] Countdown timer for voting (configure the length via .env)
 - [ ] Reveal results & set estimate via Jira API

** TODO Websocket coordination
   :LOGBOOK:
   - State "TODO"       from              [2024-08-28 Wed 23:25]
   :END:
 
- [ ] Synhronize current screen
- [ ] Listen & display voting results
