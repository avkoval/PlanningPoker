...
** DONE use iframe for docs part
   ...
   :END:
** TODO Search for ticket (Estimate ticket screen) [12/14]             :verb:
   :LOGBOOK:
 ...
 - [ ] indicate that query is running, also which query is running! with a color and sticker! and some effect!
 - [ ] avoid 2 staged query/re-render for search term 'payment`
...
** TODO Keep tickets list? refresh the same criterial? add (x) to the right?
   :LOGBOOK:
   - State "TODO"       from              [2024-09-16 Mon 20:20]
   :END:
** WORKING Voting screen [9/13] [69%]
   :LOGBOOK:
 ...
 - [X] Reveal results
 - [ ] Set estimate via Jira API
 - [-] Rewrite widget to split voting into 3 categories: Frontend, Backend and QA
   - [X] Splitting into categories in voting box & backend
   - [ ] Splitting into categories in results box
   - [ ] Make it possible to choose: SP or Hours
...
** TODO Websocket coordination [9/10]
   :LOGBOOK:
